...
BOOST_AUTO_TEST_CASE( call_order_update_test )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== call_order_update_test ===" );

      ACTORS((dan)(sam));
      const auto& bitusd = create_bitasset("USDBIT", sam.id);
      const auto& core   = CORE_ASSET(db);
...
{ try {
      ...
      BOOST_TEST_MESSAGE( "=== margin_call_limit_test ===" );
...
{ try {
      ...
      BOOST_TEST_MESSAGE( "=== black_swan_issue_346 ===" );
...
{ try {
      ...
      BOOST_TEST_MESSAGE( "=== prediction_market ===" );
...
BOOST_AUTO_TEST_CASE( create_account_test )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== create_account_test ===" );
...
BOOST_AUTO_TEST_CASE( update_account )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== update_account ===" );
...
BOOST_AUTO_TEST_CASE( transfer_core_asset )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== transfer_core_asset ===" );

      INVOKE(create_account_test);

      account_id_type committee_account;
      asset committee_balance = db.get_balance(account_id_type(), CORE_ASSET);
      ...
      BOOST_CHECK_EQUAL(get_balance(account_id_type()(db), CORE_ASSET(db)),
                        (committee_balance.amount - 10000 - fee.amount).value);
      committee_balance = db.get_balance(account_id_type(), CORE_ASSET);

      BOOST_CHECK_EQUAL(get_balance(nathan_account, CORE_ASSET(db)), 10000);
      ...
      BOOST_CHECK_EQUAL(get_balance(nathan_account, CORE_ASSET(db)), 8000 - fee.amount.value);
      BOOST_CHECK_EQUAL(get_balance(account_id_type()(db), CORE_ASSET(db)), committee_balance.amount.value + 2000);
...
BOOST_AUTO_TEST_CASE( create_committee_member )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== create_committee_member ===" );
...
BOOST_AUTO_TEST_CASE( create_mia )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== create_mia ===" );

      const asset_object& bitusd = create_bitasset( "USDBIT" );
      BOOST_CHECK(bitusd.symbol == "USDBIT");
      BOOST_CHECK(bitusd.bitasset_data(db).options.short_backing_asset == CORE_ASSET);
...
BOOST_AUTO_TEST_CASE( update_uia )
{
   ...
   try {
      ...
      //Cannot change issuer to same as before: gcc-7.4 has warning
...
BOOST_AUTO_TEST_CASE( transfer_uia )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== transfer_uia ===" );
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== create_buy_uia_multiple_match_new ===" );
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== create_buy_exact_match_uia ===" );
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== create_buy_uia_multiple_match_new_reverse ===" );
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== create_buy_uia_multiple_match_new_reverse_fract ===" );
...
BOOST_AUTO_TEST_CASE( uia_fees )
{
   try {
      ...
      BOOST_CHECK_EQUAL(get_balance(nathan_account, CORE_ASSET(db)), 0);
      transfer(committee_account, nathan_account, asset(20));
      BOOST_CHECK_EQUAL(get_balance(nathan_account, CORE_ASSET(db)), 20);
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== cancel_limit_order_test ===" );
   ...
   BOOST_CHECK_EQUAL( get_balance(buyer_account, CORE_ASSET(db)), 10000 );
...
BOOST_AUTO_TEST_CASE( witness_feeds )
{
   ...
   try {
      ...
      BOOST_TEST_MESSAGE( "=== witness_feeds ===" );
      ...
      BOOST_REQUIRE_EQUAL(active_witnesses.size(), (size_t)10);
...
BOOST_AUTO_TEST_CASE( trade_amount_equals_zero )
{
   try {
      ...
      BOOST_TEST_MESSAGE( "=== trade_amount_equals_zero ===" );
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== witness_pay_test ===" );
   ...
   BOOST_CHECK_LT( witness_ppb * 2, (uint64_t)ref_budget );
   BOOST_CHECK_GT( witness_ppb * 3, (uint64_t)ref_budget );
   ...
   sign( trx, init_account_priv_key );
   PUSH_TX( db, trx );
   uint32_t pay_fee_time = db.head_block_time().sec_since_epoch();
   trx.clear();
   ...
   while( db.head_block_time().sec_since_epoch() - pay_fee_time < 24u * block_interval )
   {
      generate_block();
      BOOST_CHECK_EQUAL( last_witness_vbo_balance().value, 0 );
   }
   ...
   BOOST_CHECK(core->reserved(db).value == 0);
   BOOST_CHECK(get_balance(*nathan, *core) == (10000 * prec));
...
BOOST_AUTO_TEST_CASE( reserve_asset_test )
{
   try
   {
      ...
      const auto& casset = CORE_ASSET(db);
...
BOOST_AUTO_TEST_CASE( cover_with_collateral_test )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "=== cover_with_collateral_test ===" );
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "=== vesting_balance_create_test ===" );
